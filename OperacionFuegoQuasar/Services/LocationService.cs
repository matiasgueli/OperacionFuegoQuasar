...
namespace OperacionFuegoQuasar.Services
{
    ...
    {
        /// <summary>
        /// Obtiene la posición de la nave a través del algoritmo de la trilateración
        /// https://github.com/TBMSP/Trilateration
        /// </summary>
        /// <param name="satellites">Lista de satélites</param>
        /// <returns>Posición de la nave</returns>
        public static double[] GetLocation(List<SatelliteData> satellites)
        {
            Point p1 = new Point(Satellite.GetSatelliteData("Kenobi").Position.Latitude, 
                                    Satellite.GetSatelliteData("Kenobi").Position.Longitude,
                                    satellites.Where(x => x.Name.ToLower() == "kenobi").Select(x => x.Distance).Single());
            Point p2 = new Point(Satellite.GetSatelliteData("Skywalker").Position.Latitude,
                                    Satellite.GetSatelliteData("Skywalker").Position.Longitude,
                                    satellites.Where(x => x.Name.ToLower() == "skywalker").Select(x => x.Distance).Single());
            Point p3 = new Point(Satellite.GetSatelliteData("Sato").Position.Latitude,
                                    Satellite.GetSatelliteData("Sato").Position.Longitude,
                                    satellites.Where(x => x.Name.ToLower() == "sato").Select(x => x.Distance).Single());
